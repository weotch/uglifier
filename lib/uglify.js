(function(exports, global) {
    ...
    SymbolDef.prototype = {
        ...
        mangle: function() {
            if (!this.mangled_name && !this.unmangleable()) this.mangled_name = this.scope.next_mangled();
        }
    };
    AST_Toplevel.DEFMETHOD("figure_out_scope", function() {
        ...
        var tw = new TreeWalker(function(node, descend) {
            ...
            if (node instanceof AST_LabeledStatement) {
                var l = node.label;
                if (labels[l.name]) throw new Error(string_template("Label {name} defined twice", l));
                labels[l.name] = l;
                descend();
    ...
    AST_Scope.DEFMETHOD("find_variable", function(name) {
        ...
        return this.variables[name] || this.parent_scope && this.parent_scope.find_variable(name);
    });
    ...
    (function() {
        ...
        PARENS(AST_New, function(output) {
            var p = output.parent();
            if (p instanceof AST_Dot && no_constructor_parens(this, output)) return true;
        });
    ...
    (function() {
        ...
        AST_Scope.DEFMETHOD("drop_unused", function(compressor) {
            ...
            if (compressor.option("unused") && !(self instanceof AST_Toplevel) && !self.uses_eval) {
                ...
                var tt = new TreeTransformer(function before(node, descend) {
                    if (node instanceof AST_Lambda) {
        ...
        OPT(AST_Binary, function(self, compressor) {
            if (compressor.option("comparisons")) switch (self.operator) {
              ...
              case "==":
              case "!=":
                if (self.left instanceof AST_UnaryPrefix && self.left.operator == "typeof" && self.right instanceof AST_String && self.right.value == "undefined") {
                    if (!(self.left.expression instanceof AST_SymbolRef) || !self.left.expression.undeclared()) {
                        self.left = self.left.expression;
                        self.right = make_node(AST_Undefined, self.right).optimize(compressor);
                        if (self.operator.length == 2) self.operator += "=";
                    }
                } else if (self.left instanceof AST_String && self.left.value == "undefined" && self.right instanceof AST_UnaryPrefix && self.right.operator == "typeof") {
